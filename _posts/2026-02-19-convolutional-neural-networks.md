...
Asssume we have a set of discrete classes (labels), then given an image, design a classifier \\(f\\) to determine which class it belongs to. Consider the following example. Let the set of classes be \\( \mathcal{C} = \{ \text{cat}, \text{rabbit}, \text{dog} \} \\).. The classifier \\(f\\) should be able to determine
