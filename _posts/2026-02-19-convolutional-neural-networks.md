...
Asssume we have a set of discrete classes (labels), then given an image, design a classifier \\(f\\) to determine which class it belongs to. Consider the following example. Set of classes 
\\(=\left\{\text{cat, dog, rabbit}\right\}\\). The classifier \\(f\\) should be able to determine
